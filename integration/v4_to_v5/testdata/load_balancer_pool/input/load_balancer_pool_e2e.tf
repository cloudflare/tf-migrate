...
resource "cloudflare_load_balancer_pool" "e2e_comprehensive" {
  ...
  # Origins configuration (v4 syntax - will be migrated to v5)
  origins {
    name    = "origin-1"
    address = "192.0.2.1"
    enabled = true
  ...
  # Load shedding configuration (v4 block, v5 attribute)
  load_shedding {
    default_percent = 55
  ...
  # Origin steering configuration (v4 block, v5 attribute)
  origin_steering {
    policy = "random"
  }
}
