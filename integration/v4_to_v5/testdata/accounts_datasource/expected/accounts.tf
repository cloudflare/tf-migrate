...
output "first_account_id" {
  value = data.cloudflare_accounts.by_name.result[0].id
}

output "first_account_name" {
  value = data.cloudflare_accounts.by_name.result[0].name
}

output "all_account_ids" {
  value = [for a in data.cloudflare_accounts.all.result : a.id]
}
