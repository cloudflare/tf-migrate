...
resource "cloudflare_device_posture_integration" "map_integrations" {
  for_each = {
    workspace_one = {
      type        = "workspace_one"
...
resource "cloudflare_device_posture_integration" "count_integrations" {
  count = 3
  ...
  type       = "custom_s2s"
  interval   = "1h"
  identifier = "legacy-identifier-${count.index}"
...
resource "cloudflare_device_posture_integration" "lifecycle_test" {
  account_id = var.cloudflare_account_id
...
resource "cloudflare_device_posture_integration" "function_test" {
  account_id = var.cloudflare_account_id
...
resource "cloudflare_device_posture_integration" "all_fields" {
  account_id = var.cloudflare_account_id
...
resource "cloudflare_device_posture_integration" "dynamic_test" {
  account_id = var.cloudflare_account_id
