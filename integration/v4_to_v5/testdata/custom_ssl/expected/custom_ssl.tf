...
# Pattern 6: geo_restrictions with us-t5 region
resource "cloudflare_custom_ssl" "us_t5_geo" {
  zone_id = var.cloudflare_zone_id
...
resource "cloudflare_custom_ssl" "count_based" {
  ...
  certificate   = var.cert_pem
  private_key   = var.key_pem
  bundle_method = element(["ubiquitous", "force", "user_defined"], count.index)
  type          = "legacy_custom"
}
...
# Pattern 13: Keyless certificate type
resource "cloudflare_custom_ssl" "keyless" {
  zone_id = var.cloudflare_zone_id
...
# Pattern 14: user_defined bundle method
resource "cloudflare_custom_ssl" "user_defined_bundle" {
  zone_id = var.cloudflare_zone_id
